...
> **In this repository.** Everything runs as Docker Compose services on **one
> VM**. Kubernetes, ECS and Swarm were deliberately dropped — see ADR 5,
> `docs/architecture/decisions/0005-run-on-one-vm-with-docker-compose.md` — so
> the orchestration advice below is context, not direction. Two invariants are
> enforced by CI and must not be broken: **no service publishes a host port**,
> and **coturn is the only host-network service**.
# Docker Patterns
...
# Upstream note: Kubernetes, ECS or Docker Swarm for production multi-container
# workloads. This repository deliberately does none of those -- one VM, Compose,
# see ADR 5. Scale here is vertical, and that is a recorded trade-off.
