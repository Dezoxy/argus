...
> `docs/threat-models/db-backup.md` §7 for the rationale.
...
- **Private signing key** — `argus-backup-signing-key` (Ed25519 PKCS8 PEM) in
  **Key Vault**, generated by `populate-keyvault.sh` (or the manual one-liner in
  `docs/threat-models/session-tokens.md §invariant-4` with this name). It is
...
> freshness). Threat model: `docs/threat-models/db-backup.md`; the
> Ed25519-signing-outside-`packages/crypto` boundary follows the ratified
> precedent in `docs/threat-models/session-tokens.md §invariant-4`.
