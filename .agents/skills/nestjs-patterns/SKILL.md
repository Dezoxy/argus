...
> **In this repository.** NestJS 11 on Node 24, strict TypeScript, ESM. The
...
> and never decrypts message content. Every new or changed controller needs a
> spec pinning its `@Public`/guard posture and status contract via
> `apps/api/src/common/testing/route-meta.ts`. Upstream text below is
> unmodified except where it named an ORM this repo does not use.
...
- Isolate transactional workflows in services that own the unit of work. In this repo that means an explicit transaction whose tenant identifier is set on the connection, which is why the DB layer is SQL-first rather than an ORM that hides the connection.
