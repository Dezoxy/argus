...
> **In this repository.** React 19 with Vite 8, built as an installable PWA.
> There is **no Next.js, no App Router, no RSC and no server components** — the
> sections below covering those describe a stack this repo does not use, and
> are kept for reference only. Routing is React Router. Every cryptographic
> operation happens in this client: MLS group state, device keys, attachment
> encryption and passkey ceremonies. Plaintext and message keys never leave
> it.
# React Patterns
...
See [`.claude/rules/ecc/react/hooks.md`](https://github.com/Dezoxy/secmes/blob/main/.claude/rules/ecc/react/hooks.md) for the full ruleset. Highlights:
...
- Run `axe` in component tests (the `react-testing` skill is not imported here; this repository uses Vitest plus the Playwright suite under `apps/web/e2e/`)
...
- Rules: [`.claude/rules/ecc/react/`](https://github.com/Dezoxy/secmes/blob/main/.claude/rules/ecc/react/coding-style.md) — coding-style, hooks, patterns, security, testing
- Skills in this repository: [`accessibility`](https://github.com/Dezoxy/secmes/blob/main/.claude/skills/accessibility/SKILL.md), [`vite-patterns`](https://github.com/Dezoxy/secmes/blob/main/.claude/skills/vite-patterns/SKILL.md), [`e2e-testing`](https://github.com/Dezoxy/secmes/blob/main/.claude/skills/e2e-testing/SKILL.md)
- Agents in this repository: `build-error-resolver` for build and type errors, `type-design-analyzer` for prop and state shapes
- Not imported here: `react-performance`, `frontend-patterns`, `react-testing`, `angular-developer`, `react-reviewer` and the `/react-*` commands. They exist in agent-base if needed.
