...
> **In this repository.** Vite 8, with `vite-plugin-pwa` for the service
> worker. The built `dist/` is baked into the Caddy ingress image at build time
> and served from the same origin as the API, so the deployment advice below
> about static hosts does not apply. The Content-Security-Policy pins the
> attachment bucket's exact origin, which constrains what the client may fetch
> — check `infra/stack/caddy/Caddyfile` before adding an external origin.
# Vite Patterns
