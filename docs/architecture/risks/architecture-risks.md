...
| RISK-007 | Dependency vulnerabilities go unaddressed because the security gate is permanently red. | Observed. The OSV scan has failed on `main` continuously, reporting 17 packages affected by 41 advisories. | None yet. | A gate that is always red is a gate nobody reads, and it currently blocks the merge rule in AGENTS.md that requires `security` green. | Open |
...
run. RISK-004 with [TD-001](https://github.com/Dezoxy/secmes/blob/main/docs/architecture/risks/technical-debt.md) is the pair to fix first, and
RISK-007 is the one blocking the review process itself.
