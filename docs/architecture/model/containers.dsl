...
argus = softwareSystem "Argus" "Invite-only, end-to-end-encrypted messenger, installable as a PWA. The server stores and forwards ciphertext and holds no key that can read a message." {
    ...
    group "Application" {
        ...
        api = container "API" "One process serving the REST API and the WebSocket gateway. Routes ciphertext, holds identity, the key directory, friends, calls, admin and GDPR. Never decrypts message content." "NestJS 12, Node 26" "Layer App"
    }
