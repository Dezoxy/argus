...
| `argus-admin-bootstrap-hash` | Argon2id hash (JSON) for the emergency **breakglass** admin login (`pnpm --filter @argus/api generate-admin-hash`). Absent ⇒ `/auth/breakglass/login` returns 503; the rest of the API is unaffected. See `docs/security/threat-models/breakglass-admin.md`. |
