...
| 1:1 voice calling (VoIP) | Art. 6(1)(b) — contract | E2EE audio (DTLS-SRTP, server crypto-blind); only call-routing **metadata** and transient relay 5-tuples are processed. V1 persists **no** call record. See `docs/security/threat-models/voip-calling.md` + `dpia-voip-calling.md`. |
...
holds no media key). See `docs/security/threat-models/voip-turn.md`.
...
| Audit events | 90 days from event creation (rolling window, enforced by the `argus-audit-prune` systemd-timer worker; see `docs/security/threat-models/audit-logging.md`) | Proportionality + Art. 6(1)(f) |
