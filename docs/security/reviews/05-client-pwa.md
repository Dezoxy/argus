...
| OBS-2 | P3 | 3 | `useMessageSending.ts:151-157`, `useConversationBackfill.ts:217,325`, `useLiveConversations.ts:314`, `join.ts:227` (+ `messaging.ts`, `ChatScreen.tsx`) | _Downgraded (real but trivial; no demonstrated trigger, no off-device sink)._ The `err instanceof Error ? err.message : err` else-branch (present at **every** live `console.warn` catch, not just one) passes a raw non-Error throw to `console.warn`. All app crypto/api layers throw `Error`, so the branch is effectively unreachable; `content` is never attached to a thrown value; devtools-console-only on a device the attacker already controls. **Spun off** (code change, bundled with SW-2 as a client-hygiene PR — this evidence PR is docs-only): normalize the else-branch to `String(err)` at **all** the sites above so the console contract is uniformly "fixed string + id + short message". |
