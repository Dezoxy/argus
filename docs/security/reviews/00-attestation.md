...
> slice notes under `docs/security/reviews/0{1..6}-*.md` are authoritative for the
> per-claim evidence.
...
Art.30 compliance overclaims verbatim — the `docs/compliance/article-30-records.md:78`
...
| 2 | **No secret/plaintext logging or persistence** (keys, tokens, full `Authorization`, presigned URLs) | **PARTIAL** | **Logging clause PROVEN** (logs carry IDs/metadata only across API, gateway, edge, error-sink — default-deny); **persistence clause violated** by one bounded, host-root-gated case — the cloudflared tunnel token sits in the container `environment:` block, so it persists in Docker daemon metadata (INF-4). PARTIAL until INF-4 is fixed, matching Slice 06's own verdict. | 01, 02, 04, 06 |
...
| 4 | **No hand-rolled crypto** (all crypto in `packages/crypto` via vetted libs; CSPRNG only) | **PASS** | All crypto confined to `packages/crypto` via vetted libraries — **ts-mls** for the MLS message/group layer, **WebCrypto AES-256-GCM** for at-rest/attachment sealing (`seal.ts`), **`@noble`** Ed25519 for device proofs; none hand-rolled. CSPRNG everywhere (`Math.random` zero functional uses); Semgrep enforces the boundary; key-substitution resistance proven. | 01, 03 |
...
Art.30 record (`docs/compliance/article-30-records.md`):** the most severe is the
...
| **F1/AR-1 — unbounded audit/session PII + ER-1 erasure gap** | P2 + P3 | **YES** | Pseudonymous metadata (probed argus-ids, refresh IPs) accumulates forever in the live DB and every backup; Art.17 erasure is actor-scoped so an erased user's argus-id survives as a *lookup target* (ER-1); and the Art.30 record formally attests a prune worker that does not exist (a DPA-facing compliance misstatement). Bounded in *kind* (never content/keys; invariants 2/3 hold) but not in *time*. Close: the prune job + extend erasure to scrub `targetArgusId` + correct the Art.30 record. (Beta-blocking, matching §7 must-fix #4.) |
...
| **INF-4 — cloudflared token in container env (persists at rest + env-delivered)** | P3 | **YES (one-line fix)** | Severity is low on its own (host-root/docker-group readable only; a network-ingress credential, not an E2EE key) — but it is the **sole reason Invariants 2 and 5 are PARTIAL**, and those are hard rules, so the trivial fix is a ship condition. Close: cloudflared credentials-file mode (mounted Docker secret) instead of the `environment:` block — flips both invariants to clean PASS. |
...
4. **F1/AR-1** — ship the audit/session prune **and** correct the Art.30 record:
   the `docs/compliance/article-30-records.md:78` "enforced by cleanup worker"
...
`docs/security/reviews/01-crypto-core.md`, `docs/security/reviews/02-server-boundary.md`,
`docs/security/reviews/03-auth-identity.md`, `docs/security/reviews/04-metadata-privacy.md`,
`docs/security/reviews/05-client-pwa.md`, `docs/security/reviews/06-infra-deploy.md`. *Campaign
plan:* `docs/planning/security-review-campaign-plan.md`. *The compliance-grade
overclaims:* `docs/compliance/article-30-records.md` (`:78` cleanup-worker
