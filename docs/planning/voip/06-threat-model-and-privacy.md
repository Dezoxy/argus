...
| A2 | **Media keys** (DTLS-SRTP master secrets) | Critical | Negotiated in-browser; MLS-bound via exporter (V1.1, see [01](./01-architecture-and-crypto-model.md)); never serialized to server |
...
| A8 | **TURN credentials** (shared secret / time-limited HMAC creds) | High | Key Vault → credential file on VM; ephemeral per-session creds to clients (TTL 600s — see [09](./09-decision-log-and-open-questions.md) Q6) |
...
| **I — IP-to-peer (direct only, V1.1)** | If a user opts into direct P2P, host/srflx candidates expose their real IP to the other peer | ADV-PEER | **Not present in V1** (relay-only, no toggle). When direct P2P ships in V1.1 it is strictly opt-in per-user; relay-only stays the default exactly because ICE leaks IPs to peers ([WebRTC Metadata and IP Leakage, arXiv 2510.16168](https://arxiv.org/abs/2510.16168)). UI must state plainly: "the other person will see your IP address." Both peers' settings considered — **relay-only wins** if *either* side requires it | Opt-in users accept peer IP exposure knowingly. **Accepted by user choice** (V1.1) |
| **I — TURN logs** | coturn can log allocations, 5-tuples, usernames, credentials | ADV-SRV | Run coturn with `--no-stdout-log` and a `/dev/null` (or syslog-minimal) log target; **no verbose/`-v`**; never log credentials; logs (if any) carry no message-derived data and follow short retention ([coturn turnserver manpage](https://manpages.debian.org/testing/coturn/turnserver.1.en.html), [Securing coturn – Enable Security](https://www.enablesecurity.com/blog/coturn-security-configuration-guide/)) | Live process memory still holds 5-tuples during a call. **Accepted** with minimized logging |
...
| **S/T — DTLS fingerprint substitution (MITM)** | Active attacker rewrites the SDP fingerprint in transit | ADV-SRV, ADV-NET | Signaling rides the **authenticated WSS** gateway; SDP/fingerprint carried **inside MLS-encrypted payload** so the crypto-blind server can't read or alter it. The detectable binding requires (a) the **new authenticated-sender decrypt path** in `packages/crypto` so the recipient knows *which* MLS member sent the offer, and (b) **MLS-exporter binding** of the DTLS fingerprint to that identity ([01](./01-architecture-and-crypto-model.md)). Secure signaling channel is the documented baseline defence ([WebRTC Security: DTLS-SRTP – antmedia.io](https://antmedia.io/webrtc-security/)) | Lowered to *detectable* once the authenticated-sender path + exporter binding land. **Needs-work** — the authenticated-sender path is a **Phase-0 blocker**; exporter binding is V1.1 (Q5: exporter key = no for V1, async shim — see [09](./09-decision-log-and-open-questions.md)) |
...
| **R — Repudiation** | "I never called you" | n/a | **Out of scope** — argus is privacy-first, not an audit/non-repudiation system. No durable call ledger in V1 by design (§10 non-goals) | Intentional. **Accepted** |
...
| **E — Privilege/relay abuse** | Use TURN as an open relay to internal services or third parties | ADV-ABUSE, external | coturn **time-limited credentials** (REST/HMAC, 600s TTL), `--no-multicast-peers`, **deny RFC1918/internal ranges**, `--denied-peer-ip` for the VM's own subnet; isolate TURN on its own security group ([TURN Server Security Best Practices – Enable Security](https://www.enablesecurity.com/blog/turn-security-best-practices/)) | Low with correct config. **Needs-work** (config must be reviewed by `infra-reviewer`) |
| **E — Admin path to content/metadata** | Ops surface exposing call detail | ADV-SRV insider | Admin surfaces stay **metadata-only** (invariant 6); no call content ever, no recording (§10) | None if invariant held. **Accepted** |
...
| **No call-history enumeration** | Wrong-caller / non-friend attempts return uniform responses (reuse the friends-module "uniform 202 / uniform 404, no oracle" posture). | **Should** |
...
| R6 | **DTLS-fingerprint MITM** by active server/network attacker | Medium-High | WSS signaling + SDP inside MLS ciphertext; **new authenticated-sender decrypt path** (Phase-0 blocker) + exporter-binding (V1.1) make substitution *detectable* | **Needs-work** — authenticated-sender path is a hard Phase-0 predecessor of the first connecting call; exporter binding follows in V1.1 ([01](./01-architecture-and-crypto-model.md)) |
| R7 | **Public TURN port** breaks zero-ingress model; first internet-facing attack surface | High | Strict NSG/SG rules, isolated relay, coturn hardening, CI compose-guard exception modeled explicitly | **Needs-work** — resolved in [03](./03-infrastructure-turn-and-networking.md); requires `infra-reviewer` sign-off |
| R8 | **Relay abuse / open-relay** to internal or third-party hosts | High | Time-limited HMAC creds (600s TTL), deny RFC1918, `--no-multicast-peers`, isolation | **Needs-work** — config review required ([Enable Security](https://www.enablesecurity.com/blog/turn-security-best-practices/)) |
| R9 | **Spam-calling / ring-flood / harassment** | Medium | Friendship gate + per-socket rate limit + block + coturn quotas | **Needs-work** — quota tuning + block enforcement to implement |
...
| **Single shared VM (SPOF)** | A VM-level failure takes down API, gateway, *and* relay together. | **Accepted for V1.** Ingress Option (d) — a dedicated relay host/IP ([03](./03-infrastructure-turn-and-networking.md), [09](./09-decision-log-and-open-questions.md) Q1) — is the HA lever, slated to become the default before video. |
| **Ephemeral TURN cred expiry mid-setup** | A 600s-TTL credential ([09](./09-decision-log-and-open-questions.md) Q6) that expires before ICE completes fails the allocation. | 600s comfortably exceeds setup time; client requests fresh creds per call attempt. |
...
### Sources
- [WebRTC Metadata and IP Leakage in Modern Browsers (arXiv 2510.16168)](https://arxiv.org/abs/2510.16168)
- [WebRTC Leaks: A Complete Guide – Security.org](https://www.security.org/vpn/webrtc-leak/)
- [WebRTC Security in 2025: Protocols, Vulnerabilities, and Best Practices – WebRTC.ventures](https://webrtc.ventures/2025/07/webrtc-security-in-2025-protocols-vulnerabilities-and-best-practices/)
- [WebRTC Security: DTLS-SRTP, Encryption, and Token Authorization – antmedia.io](https://antmedia.io/webrtc-security/)
- [TURN Server Security Best Practices – Enable Security](https://www.enablesecurity.com/blog/turn-security-best-practices/)
...
- [coturn turnserver(1) manpage – Debian](https://manpages.debian.org/testing/coturn/turnserver.1.en.html)
